...
impl Solution {
    ...
    fn solve_second_puzzle(input_file: &str) -> Vec<char> {
        let lines = file_io::convert_file_to_lines_of_string(input_file);
        const MOVE_KEY: &str = "move";
        let move_operation_line_start_number = lines
            .iter()
        ...
        move_operations
            .iter()
            .for_each(|op| op.apply_to_in_order(&mut initial_stacks));
...
mod tests {
    ...
    fn solution_for_second_puzzle() {
        let solution: Vec<char> = Solution::solve_second_puzzle("inputs/day_5/input.txt");
        let expected: Vec<char> = vec!['G', 'S', 'L', 'C', 'M', 'F', 'B', 'R', 'P'];
        assert_eq!(expected, solution);
    }
}
